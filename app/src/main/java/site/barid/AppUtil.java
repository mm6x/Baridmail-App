...
public class AppUtil {
    ...
    public static void showMessage(Context _context, String _s) {
        if (_context instanceof Activity) {
            showIOSDialog((Activity) _context, "Alert", _s);
        } else {
            Toast.makeText(_context, _s, Toast.LENGTH_SHORT).show();
        }
    }

    public static void showIOSDialog(final Activity activity, String title, String message) {
    ...
    public static String getFriendlyErrorMessage(Context context, String errorMessage) {
        if (!isConnected(context)) {
            return "No internet connection. Please check your network settings.";
        }
        if (errorMessage.contains("UnknownHostException") || errorMessage.contains("Unable to resolve host")) {
            return "Unable to connect to server. Please try again later.";
        }
        if (errorMessage.contains("ConnectException") || errorMessage.contains("Failed to connect")) {
            return "Could not connect to the server. It might be down.";
        }
        if (errorMessage.contains("SocketTimeoutException") || errorMessage.contains("timeout")) {
        ...
        if (errorMessage.contains("500") || errorMessage.contains("502") || errorMessage.contains("503")) {
            return "Server error. We are working on it.";
        }
