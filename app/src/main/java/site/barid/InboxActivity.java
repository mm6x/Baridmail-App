...
public class InboxActivity extends AppCompatActivity {
	...
	private RequestNetwork deleteRequest;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		deleteRequest = new RequestNetwork(this); // Initialized deleteRequest
		
		binding.swiperefreshlayout1.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
			@Override
			public void onRefresh() {
				vibrator.vibrate((long)(50));
				AccountManager accountManager = new AccountManager(InboxActivity.this);
		...
		listRequest_listener = new RequestNetwork.RequestListener() {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				try{
					...
					if (emailsResponse != null && emailsResponse.get("result") != null) {
						Map<String, Object> result = (Map<String, Object>) emailsResponse.get("result");
						Object itemsObj = result.get("items");
						if (itemsObj != null) {
							emailList = new Gson().fromJson(new Gson().toJson(itemsObj), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
			...
			public void onErrorResponse(String _param1, String _param2) {
				...
				if (_message.contains("401") || _message.toLowerCase().contains("unauthorized")) {
					AppUtil.showIOSDialog(InboxActivity.this, "Authentication Failed", getString(R.string.error_auth_failed));
				} else {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				try {
					...
					if (emailDetailResponse != null && emailDetailResponse.get("result") != null) {
						...
						String subject = item.get("subject") != null ? String.valueOf(item.get("subject")) : "(No Subject)";
						
						String from = item.get("fromAddress") != null ? String.valueOf(item.get("fromAddress")) : "";
						if (from.isEmpty() || from.equals("null")) {
							from = item.get("from_address") != null ? String.valueOf(item.get("from_address")) : "Unknown";
						}
						
						String body = item.get("textContent") != null ? String.valueOf(item.get("textContent")) : "";
						if (body.isEmpty() || body.equals("null")) {
							body = item.get("htmlContent") != null ? String.valueOf(item.get("htmlContent")) : "";
						}
						// Strip HTML if it's the fallback but we only have a TextView for now
						if (body.contains("<") && body.contains(">")) {
							body = android.text.Html.fromHtml(body).toString();
						}
		...
		if (!accountManager.hasAccounts()) {
			...
			if (!legacyEmail.isEmpty() && legacyEmail.contains("@")) {
				...
				if (atIndex > 0) {
					...
					accountManager.addAccount(userPart, domainPart, null);
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public void filter(String query) {
			...
			if (query == null || query.isEmpty()) {
				...
				for (HashMap<String, Object> item : _originalData) {
					String from = item.get("fromAddress") != null ? item.get("fromAddress").toString().toLowerCase() : "";
					String subject = item.get("subject") != null ? item.get("subject").toString().toLowerCase() : "";
					// Also search snippet if available
					String snippet = item.get("textContent") != null ? item.get("textContent").toString().toLowerCase() : "";
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			String from = _data.get((int)_position).get("fromAddress").toString();
			String subject = _data.get((int)_position).get("subject") != null ? _data.get((int)_position).get("subject").toString() : "";
